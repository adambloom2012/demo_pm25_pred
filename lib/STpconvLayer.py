...
class STpconv(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size=(3, 3, 3), strides=(1, 1, 1), use_bias=True, activation=None):
        ...
        if activation is None:
            ...
            raise ValueError(
                "Failed to interpret activation arg, expected None, string or instance of tf.keras.layers.Layer.")
    ...
    def build(self, input_shape):
        ...
        if len(input_shape) != 2:
            raise ValueError(
                "Invalid input shape, expected length-two list containing image and mask.")
        if input_shape[0][channel_axis] is None:
            raise ValueError(
                "The channel dimension of image inputs should be defined. Found `None`")
        if input_shape[1][channel_axis] is None:
            raise ValueError(
                "The channel dimension of mask inputs should be defined. Found `None`.")
        ...
        if self.use_bias:
            self.bias = self.add_weight(
                shape=(self.filters,), initializer=tf.keras.initializers.zeros(), name='bias')
        else:
            self.bias = None
        self.built = True
    ...
    def call(self, inputs):
        ...
        if type(inputs) is not list or len(inputs) != 2:
            raise Exception(
                'STpconv must be called on a list of two tensors [img, mask]. Instead got: ' + str(inputs))

        # Padding (=zero padding) done explicitly so that padding becomes part of the masked partial convolution
        imgs = tf.pad(inputs[0], self.paddings, "CONSTANT",
                      constant_values=0)  # zero padding
        masks = tf.pad(inputs[1], self.paddings, "CONSTANT",
                       constant_values=0)  # zero padding
        ...
        mask_output = tf.nn.conv3d(masks, self.kernel_mask, strides=(
            1,) + self.strides + (1,), padding="VALID")  # TODO: data format?
        img_output = tf.nn.conv3d(masks * imgs, self.kernel, strides=(
            1,) + self.strides + (1,), padding="VALID")  # TODO: data format?
