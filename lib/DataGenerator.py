...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, input_dir, batch_size=32, dim=(128, 128, 16), n_channels=1, shuffle=True, nmax=-1, fill=0):
        ...
        assert len(self.mask_files) == len(self.y_files) and len(self.mask_files) == len(
            self.x_files) and len(self.val_mask_files) == len(self.x_files)
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_size):
            ...
            # set nodata values to 0 because masked pixels otherwise do contribute to the kernel sums
            temp[temp < 0] = self.fill
            print(f"  – existing X shape: {X.shape}")
            print(f"  – new temp  shape: {temp.shape}")
