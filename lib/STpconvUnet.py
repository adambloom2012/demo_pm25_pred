...
class STpconvUnet(object):
    ...
    def __init__(self, n_conv_layers=4, nx=128, ny=128, nt=16, inference_only=False, net_name='default',
        ...
        if not kernel_sizes is None:
            assert len(
                kernel_sizes) == n_conv_layers, "len(kernel_sizes) must equal n_conv_layers"

        if not n_filters is None:
            assert len(
                n_filters) == n_conv_layers, "len(n_filters) must equal n_conv_layers"

        if not strides is None:
            assert len(
                strides) == n_conv_layers, "len(strides) must equal n_conv_layers"
    ...
    def build_pconv_unet(self):
        ...
        inputs_img = tf.keras.Input(
            (self.nx, self.ny, self.nt, 1), name='inputs_img')
        inputs_mask = tf.keras.Input(
            (self.nx, self.ny, self.nt, 1), name='inputs_mask')
        input_validation_mask = tf.keras.Input(
            (self.nx, self.ny, self.nt, 1), name='validation_mask')

        # ENCODER
        def encoder_layer(img_in, mask_in, filters, kernel_size, strides):
            if (self.n_conv_per_block > 1):
                conv, mask = STpconv(filters, kernel_size, strides=(
                    1, 1, 1), activation=tf.keras.layers.LeakyReLU(alpha=0.1))([img_in, mask_in])
                if (self.n_conv_per_block > 2):
                    for i in range(self.n_conv_per_block - 2):
                        conv, mask = STpconv(filters, kernel_size, strides=(
                            1, 1, 1), activation=tf.keras.layers.LeakyReLU(alpha=0.1))([conv, mask])
                conv, mask = STpconv(filters, kernel_size, strides=strides,
                                     activation=tf.keras.layers.LeakyReLU(alpha=0.1))([conv, mask])
            else:
                conv, mask = STpconv(filters, kernel_size, strides=strides,
                                     activation=tf.keras.layers.LeakyReLU(alpha=0.1))([img_in, mask_in])
        ...
        e_mask = []
        for i in range(self.n_conv_layers):
            prev_img, prev_mask = encoder_layer(
                prev_img, prev_mask, self.n_filters[i], self.kernel_sizes[i], self.strides[i])
        ...
        def decoder_layer(img_in, mask_in, e_conv, e_mask, filters, kernel_size, size):
            ...
            concat_mask = tf.keras.layers.Concatenate(
                axis=4)([e_mask, up_mask])

            if (self.n_conv_per_block > 1):
                conv, mask = STpconv(filters, kernel_size)(
                    [concat_img, concat_mask])
                for i in range(self.n_conv_per_block - 1):
                    conv, mask = STpconv(filters, kernel_size, activation=tf.keras.layers.LeakyReLU(
                        alpha=0.1))([conv, mask])
            else:
                conv, mask = STpconv(filters, kernel_size, activation=tf.keras.layers.LeakyReLU(
                    alpha=0.1))([concat_img, concat_mask])
        ...
        for i in reversed(range(self.n_conv_layers)):
            if i == 0:
                prev_img, prev_mask = decoder_layer(prev_img, prev_mask, inputs_img,
                                                    # decoder layers use 3x3x3 kernels only
                                                    inputs_mask, 1, (3, 3, 3), self.strides[i])
            else:
                prev_img, prev_mask = decoder_layer(prev_img, prev_mask, e_conv[i-1],
                                                    # decoder layers use 3x3x3 kernels only
                                                    e_mask[i-1], self.n_filters[i-1], (3, 3, 3), self.strides[i])
        ...
        outputs = tf.keras.layers.Conv3D(
            1, 1, activation="linear")(d_conv[len(d_conv)-1])

        # Setup the model inputs / outputs
        model = tf.keras.Model(
            inputs=[inputs_img, inputs_mask, input_validation_mask], outputs=outputs)
    ...
    def compile_pconv_unet(self, model, inputs_mask, validation_mask):
        model.compile(
            ...
            metrics=[losses.mae_gaps(inputs_mask, validation_mask), losses.rmse_gaps(
                inputs_mask, validation_mask),  losses.r2_gaps(inputs_mask, validation_mask)]
        )
